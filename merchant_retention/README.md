...
1. **Turns change data capture (CDC) into enrollment history**: deduplicates
   ...
   its following churn, unions in a churn batch applied by a manual extract,
   transform, load (ETL) process that never hit the log, and fills open
   intervals with the anchor date.
...
dbt, then any business intelligence (BI) tool and any analyst can read the
metric off small tables with boring joins.
